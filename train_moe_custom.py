...
# -*- coding: utf-8 -*-
"""
YOLOv8n-MoE Custom Training Script with Fixed Metrics Calculation

This script trains the YOLOv8n-MoE model with proper metrics calculation
to prevent doubled precision/recall values. It includes:
- Custom training loop with MoE-specific handling
- Load balancing auxiliary loss
- Proper metrics calculation that prevents duplication
- Expert utilization tracking

Usage:
    python train_moe_custom.py --epochs 100 --batch 16
    python train_moe_custom.py --epochs 5 --batch 4  # Quick test
"""
...
class YOLOv8n_MoE_C2f(nn.Module):
    ...
    def model(self):
        ...
        # v8DetectionLoss accesses model.model[-1] to get the Detect head
        # We return a list where the last element is the Detect head
        return [self.h_22]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train YOLOv8n-MoE with fixed metrics')
    ...
    print("\n" + "="*60)
    print("  YOLOv8n-MoE Training with Fixed Metrics Calculation")
    print("="*60 + "\n")
    ...
    model.args = hyp  # Attach args for v8DetectionLoss
    ...
    # Load dataset (using Ultralytics data loading)
    ...
    # Build dataloaders
    # We use a simplified approach compatible with standard PyTorch
    ...
    train_dataset = YOLODataset(
        ...
        augment=True,
        hyp=hyp,  # Use default hyperparameters
        rect=False,
        cache=False,
        single_cls=False,
        stride=32,
    ...
    print(f"✅ Data loaded: {len(train_dataset)} images, {len(train_loader)} batches")
    
    # Save initial model
    model_save_path = os.path.join(CHECKPOINT_DIR, 'yolov8n_moe_init.pt')
    torch.save({
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'config': {'nc': NUM_CLASSES, 'E': E, 'K': K}
    }, model_save_path)
    print(f"✅ Initialized model saved to: {model_save_path}")
